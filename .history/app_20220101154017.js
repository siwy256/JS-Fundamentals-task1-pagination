...
const paginateArray = (dataEntries, settings) => {
	if(! (dataEntries instanceof Array) ) { throw new Error('first parameter "dataEntries" has to be an array type'); } 
	if(typeof settings !=='object') { throw new Error('second parameter "setings" has to be a object type'); } 
	if(! ('actualPageIdx' in settings || (! ('entriesOnPage' in settings)) ) { 
		throw new Error('second parameter "setings" has to be a object type { actualPageIdx, entriesOnPage}'); 
	} 
